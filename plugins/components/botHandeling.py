...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /stream https://t.me/VastavikMovieFlix or https://t.me/VastavikMovieFlix")
    except IndexError:
        await message.reply("Usage: /stream https://t.me/VastavikMovieFlix or https://t.me/VastavikMovieFlix")
        return
...
async def m_grp(client, message):
    links = []
    link = await client.ask(message.chat.id ,"send me your pm search grp link or send /skiplink to skip , default is VastavikMovieFlix")
    if link.text == "/skiplink":
        links.append("https://t.me/VastavikMovieFlix")
    else:
        if checkIfLinkIsValid(link.text):
            links.append(link.text)
        else:
            await message.reply("Invalid link")
    link1 = await client.ask(message.chat.id ,"send me your movies grp link or send /skiplink to skip . default is VastavikMovieFlix")
